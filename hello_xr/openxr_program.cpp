...
struct OpenXrProgram : IOpenXrProgram {
    ...
    static void LogLayersAndExtensions() {
        ...
        const auto logExtensions = [](const char* layerName, int indent = 0) {
            ...
            for (const XrExtensionProperties& extension : extensions) {
                Log::Write(Log::Level::Info, Fmt("%sAvailable Extensions:  Name=%s", indentStr.c_str(), extension.extensionName,
    ...
    void CreateInstanceInternal() {
        ...
        xrGetInstanceProperties(m_instance, &instanceInfo);

        
        Log::Write(Log::Level::Info, Fmt("Runtime %s: Version : %u.%u.%u", instanceInfo.runtimeName,
    ...
    void InitializePassthroughFB() {
        ...
        CHECK_XRCMD(xrGetInstanceProcAddr(m_instance, "xrCreatePassthroughFB", (PFN_xrVoidFunction*)(&pfnXrCreatePassthroughFB)));

        if(pfnXrCreatePassthroughFB == nullptr)
        {
            Log::Write(Log::Level::Error, "Failed to xrCreatePassthroughFB");
            return;
        }

        CHECK_XRCMD(pfnXrCreatePassthroughFB( m_session, &createInfoPass, &m_passthrough));
        ...
        createInfoLayer.next = nullptr;
        createInfoLayer.flags = layerFlags;
		createInfoLayer.passthrough = m_passthrough;
        ...
        CHECK_XRCMD(xrGetInstanceProcAddr(m_instance, "xrCreatePassthroughLayerFB", (PFN_xrVoidFunction*)(&pfnXrCreatePassthroughLayerFB)));
        if(pfnXrCreatePassthroughFB == nullptr)
        {
            Log::Write(Log::Level::Error, "Failed to xrCreatePassthroughLayerFB");
            return;
        }
        
        CHECK_XRCMD(pfnXrCreatePassthroughLayerFB( m_session, &createInfoLayer, &m_passthroughLayer));

        PFN_xrPassthroughStartFB pfnXrPassthroughStartFB = nullptr;
        CHECK_XRCMD(xrGetInstanceProcAddr(m_instance, "xrPassthroughStartFB", (PFN_xrVoidFunction*)(&pfnXrPassthroughStartFB)));
        
        if(pfnXrPassthroughStartFB == nullptr)
        {
            Log::Write(Log::Level::Error, "Failed to xrPassthroughStartFB");
            return;
        }

        CHECK_XRCMD(pfnXrPassthroughStartFB(m_passthrough));
    }

    void InitializeHandtracking() {
        PFN_xrCreateHandTrackerEXT pfnXrCreateHandTrackerEXT = nullptr;
        CHECK_XRCMD(xrGetInstanceProcAddr(m_instance, "xrCreateHandTrackerEXT", (PFN_xrVoidFunction*)(&pfnXrCreateHandTrackerEXT)));

        if(pfnXrCreateHandTrackerEXT == nullptr)
        {
            Log::Write(Log::Level::Error, "Failed to xrCreateHandTrackerEXT");
            return;
        }

        CHECK_XRCMD(xrGetInstanceProcAddr(m_instance, "xrLocateHandJointsEXT", (PFN_xrVoidFunction*)(&pfnXrLocateHandJointsEXT)));

        if(pfnXrCreateHandTrackerEXT == nullptr)
        {
            Log::Write(Log::Level::Error, "Failed to xrLocateHandJointsEXT");
            return;
        }
    ...
    void AppResumed()
    {
        Log::Write(Log::Level::Info, "AppResumed() called");
        if(m_passthroughLayer == XR_NULL_HANDLE) {
            Log::Write(Log::Level::Warning, "Resume skipping passthrough as m_passthroughLayer is null");
            return;
        }

        PFN_xrPassthroughLayerResumeFB pfnXrPassthroughLayerResumeFB = nullptr;
        CHECK_XRCMD(xrGetInstanceProcAddr(m_instance, "xrPassthroughLayerResumeFB", (PFN_xrVoidFunction*)(&pfnXrPassthroughLayerResumeFB)));
        
        if(pfnXrPassthroughLayerResumeFB == nullptr)
        {
            Log::Write(Log::Level::Error, "Failed to xrPassthroughLayerResumeFB");
            return;
        }

        CHECK_XRCMD(pfnXrPassthroughLayerResumeFB(m_passthroughLayer));
    }

    void AppPaused()
    {
        Log::Write(Log::Level::Info, "AppPaused() called");
        if(m_passthroughLayer == XR_NULL_HANDLE) {
            Log::Write(Log::Level::Warning, "Pause skipping passthrough as m_passthroughLayer is null");
            return;
        }

        PFN_xrPassthroughLayerPauseFB pfnXrPassthroughLayerPauseFB = nullptr;
        CHECK_XRCMD(xrGetInstanceProcAddr(m_instance, "xrPassthroughLayerPauseFB", (PFN_xrVoidFunction*)(&pfnXrPassthroughLayerPauseFB)));
        
        if(pfnXrPassthroughLayerPauseFB == nullptr)
        {
            Log::Write(Log::Level::Error, "Failed to xrPassthroughLayerPauseFB");
            return;
        }

        CHECK_XRCMD(pfnXrPassthroughLayerPauseFB(m_passthroughLayer));
    }
    ...
    void RenderFrame() override {
        ...
        XrCompositionLayerFlags fblayerFlags = {};
        XrCompositionLayerPassthroughFB fblayer{XR_TYPE_COMPOSITION_LAYER_PASSTHROUGH_FB};
        fblayer.next = nullptr;
        fblayer.flags = fblayerFlags;
        fblayer.space = XR_NULL_HANDLE;
        fblayer.layerHandle = m_passthroughLayer;
        layers.push_back(reinterpret_cast<XrCompositionLayerBaseHeader*>(&fblayer));
        ...
        frameEndInfo.environmentBlendMode = XR_ENVIRONMENT_BLEND_MODE_OPAQUE;//m_options.Parsed.EnvironmentBlendMode;
    ...
    bool RenderLayer(XrTime predictedDisplayTime, std::vector<XrCompositionLayerProjectionView>& projectionLayerViews,
        ...
        if(pfnXrLocateHandJointsEXT != nullptr)
        {
            for (auto hand : {m_xrHandTrackerEXTLeft, m_xrHandTrackerEXTRight}) {
                XrHandJointLocationEXT jointLocations[XR_HAND_JOINT_COUNT_EXT];
                XrHandJointLocationsEXT locationsEXT = {
                ...
                if(pfnXrLocateHandJointsEXT( hand, &locateInfoEXT, &locationsEXT) != XR_SUCCESS)
                {
                    Log::Write(Log::Level::Verbose, Fmt("Failed to get hand joint locations"));
                    continue;
                }
   ...
   private:
    ...
    XrHandTrackerEXT m_xrHandTrackerEXTLeft;
    XrHandTrackerEXT m_xrHandTrackerEXTRight;
